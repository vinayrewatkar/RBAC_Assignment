...
import { Edit, Trash2, Plus, ToggleLeft, ToggleRight, Search, AlertCircle } from 'lucide-react';
...
const UserTable: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Check for duplicates without affecting the display
    const emailCounts = users.reduce((acc: { [key: string]: number }, user) => {
      acc[user.email] = (acc[user.email] || 0) + 1;
      return acc;
    }, {});

    const hasDuplicates = Object.values(emailCounts).some(count => count > 1);
    if (hasDuplicates && !showDuplicateAlert) {
  ...
  return (
    <>
      {showDuplicateAlert && (
        <CustomAlert message="Warning: Duplicate email addresses detected. Some features might not work as expected." />
      )}
